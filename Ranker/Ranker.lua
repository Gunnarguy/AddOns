...
local hasPromotedAll
local instanceName, instanceType
...
local honorTabAvailable = true
local dataBrokerHK, dataBrokerHonor, dataBrokerCurrentHonor
local dataBrokerNeedMoreHK = false
local dataBrokerNeedMoreHonor = false
local dataBrokerMilestones = {}
local lookupData = { defaults = {
  ...
  rankerObjective = 14,
  rankerLimit = 500000,
  rankerObjectiveScenario = 14,
  rankerLimitScenario = 500000,
...
local LibDBRanker = LibStub:GetLibrary("LibDataBroker-1.1"):NewDataObject("Ranker", { type = "data source", icon = format("%s%02d", "Interface\\PvPRankBadges\\PvPRank", rankerObjective or lookupData.defaults.rankerObjective), label = "Ranker", text = "Click to open Ranker", OnClick = function (self, button)
...
end, OnEnter = function (self, button)
  GameTooltip:SetOwner(self, "ANCHOR_NONE")
  GameTooltip:SetPoint("TOPLEFT", self, "BOTTOMLEFT")
  GameTooltip:ClearLines()
  if worldLoaded and addonLoaded and interfaceLoaded then
    Ranker:UpdateDataBroker(GameTooltip)
    GameTooltip:ClearLines()
    GameTooltip:AddDoubleLine("Ranker version", rankerVersion)
    GameTooltip:AddLine(" ")
    if rankerObjective then
      GameTooltip:AddDoubleLine("Objective", "Rank " .. rankerObjective)
    end
    GameTooltip:AddDoubleLine("Currently", "Rank " .. playerRank .. " and " .. format("%.2f%%", playerRankProgress * 100))
    GameTooltip:AddDoubleLine("Weekly reset", "in " .. SecondsToTime(GetSecondsUntilWeeklyReset()))
    if #dataBrokerMilestones > 0 then
      GameTooltip:AddLine(" ")
      GameTooltip:AddLine("Milestones")
    end
    local stop = false
    for key, line in pairs(dataBrokerMilestones) do
      if line.honorNeed > dataBrokerCurrentHonor and line.rank <= rankerObjective and line.situation ~= "+1" and stop ~= true then
        if line.rank == maxObtainableRank then stop = true end
        GameTooltip:AddDoubleLine(FormatLargeNumber(line.honorNeed) .. " honor", format("Rank %d and %.2f%%", line.rank, line.rankProgress * 100), 1, 1, 1, 1, 1, 1)
        if (line.situation == "!!" and lookupData.defaults.allowDecayPreventionHop == true) or line.situation ~= "!!" and line.honorRemains > 0 then
          GameTooltip:AddDoubleLine(" ", "Need " .. FormatLargeNumber(line.honorRemains) .. " more honor", 1, 1, 1, 1, 1, 1)
        end
        if key ~= #dataBrokerMilestones then
          GameTooltip:AddLine(" ")
        end
      end
    end
    GameTooltip:AddLine(" ")
    GameTooltip:AddLine("Left click to open Ranker")
    GameTooltip:AddLine("Right click to open configuration options")
  end
  GameTooltip:Show()
end, OnLeave = function (self, button)
  GameTooltip:Hide()
end, })
...
function Ranker:SlashHandler(message, editbox)
  ...
  elseif message == "show" then
    ...
    Ranker:OutputToChat("Available commands: /ranker, /ranker show, /ranker config, /ranker prune", true)
    Ranker:ShowWindow()
    Ranker:ResetToCurrentSituationAndPredict()
  end
end
...
function Ranker:OnEventHandler(frame, event, arg1, ...)
  ...
  if event == "ADDON_LOADED" then
    ...
    if HasActiveSeason() and GetActiveSeason() == Enum.SeasonID.SeasonOfDiscovery and gameTocVersion * 1 < 20000 then
      -- Classic Era: Season of Discovery realms
      local timestampNextPhase = time({ ["day"] = 19, ["month"] = 11, ["year"] = 2024, ["hour"] = 0 })
      local timestampNextPhaseForAll = time({ ["day"] = 21, ["month"] = 11, ["year"] = 2024, ["hour"] = 0 })
      ...
      if ((timestampNow >= timestampNextPhase and GetSecondsUntilWeeklyReset() > timestampsTwoDays) or timestampNow >= timestampNextPhaseForAll) then
        if gameTocVersion * 1 >= 11505 and gameBuild * 1 >= 57537 then
        ...
        if gameTocVersion * 1 >= 11502 and gameBuild * 1 >= 55563 then
          maxObtainableRank = 10
        end
        if gameTocVersion * 1 >= 11504 and gameBuild * 1 >= 56738 then
    ...
    elseif HasActiveSeason() and (GetActiveSeason() == Enum.SeasonID.FreshHardcore or GetActiveSeason() == Enum.SeasonID.Fresh) and gameTocVersion * 1 < 20000 then
      -- Classic Era: Anniversary realms: "fresh" and "fresh hardcore"
      local timestampNextPhase = time({ ["day"] = 1, ["month"] = 4, ["year"] = 2025, ["hour"] = 0 })
      local timestampNextPhaseForAll = time({ ["day"] = 3, ["month"] = 4, ["year"] = 2025, ["hour"] = 0 })
      local timestampsTwoDays = 2 * 24 * 60 * 60
      local timestampNow = time()
      if ((timestampNow >= timestampNextPhase and GetSecondsUntilWeeklyReset() > timestampsTwoDays) or timestampNow >= timestampNextPhaseForAll) then
        if gameTocVersion * 1 >= 11506 and gameBuild * 1 >= 59415 then
          maxObtainableRank = 14
        end
      else
        if gameTocVersion * 1 >= 11505 and gameBuild * 1 >= 58555 then
      ...
      if maxObtainableRank >= 9 and maxObtainableRank < 13 then limit = maxObtainableRank + 2 end
      lookupData.defaults.rankerLimit = honorIncrements[limit]
      if rankerLimit and lookupData.defaults.rankerLimit and rankerLimit > lookupData.defaults.rankerLimit then rankerLimit = lookupData.defaults.rankerLimit end
    end
  ...
  elseif event == "PLAYER_LOGIN" then
    ...
    if rankerOptions.autoPromote == nil then
      rankerOptions.autoPromote = lookupData.defaults.autoPromote
    end
    if rankerObjective == nil then
      rankerObjective = lookupData.defaults.rankerObjective
    end
    if rankerObjectiveScenario == nil then
      rankerObjectiveScenario = lookupData.defaults.rankerObjectiveScenario
    end
    if rankerLimit == nil then
      rankerLimit = lookupData.defaults.rankerLimit
    end
    if rankerLimitScenario == nil then
      rankerLimitScenario = lookupData.defaults.rankerLimitScenario
    end
    LibDBRanker.icon = format("%s%02d", "Interface\\PvPRankBadges\\PvPRank", rankerObjective)
  ...
  elseif event == "PLAYER_ENTERING_WORLD" then
    ...
    -- Verify if the honor tab is already available or not
    ToggleCharacter("PaperDollFrame", true)
    honorTabAvailable = CharacterFrameTab5:IsVisible()
    ToggleCharacter("PaperDollFrame", false)
    if honorTabAvailable == false then
      Ranker:OutputToChat("Until the honor pane becomes available for you, open Ranker with the following command: /ranker show", true)
    end
  ...
  elseif event == "PARTY_LEADER_CHANGED" or event == "GROUP_ROSTER_UPDATE" then
    if rankerOptions.autoPromote then
      if IsInRaid() and UnitIsGroupLeader("player") and hasPromotedAll ~= true and instanceType == "pvp" then
        if not IsEveryoneAssistant() then
          SetEveryoneIsAssistant(true)
          Ranker:OutputToChat("Everyone is now promoted to raid assistant.", true)
          hasPromotedAll = true
        end
      end
    end
...
function Ranker:AddOnLoaded()
  if worldLoaded then
    ...
    if not registerPrefixSuccess then
      print("Error with RegisterAddonMessagePrefix")
      return
    end
    ...
    -- We're ready
    Ranker:OutputToChat("AddOn version " .. rankerVersion .. " loaded.", debug)
  end
end
-----
-- User interface
...
function Ranker:CreateUserInterface()
  ...
  function dropDownRankerObjective:SetValue(newValue)
    ...
    LibDBRanker.icon = rankIcon
    CloseDropDownMenus()
    Ranker:Predict()
  end
  Ranker.dropDownRankerObjective = dropDownRankerObjective
  -- Dropdown list for the player's rank objective at rank 14 for use during scenario modeling
  local dropDownRankerObjectiveMaxRank = LibDD:Create_UIDropDownMenu("RankerObjectiveMaxRank", mainFrame.ScrollChild)
  dropDownRankerObjectiveMaxRank:SetPoint("TOPLEFT", 20, -21)
  LibDD:UIDropDownMenu_SetWidth(dropDownRankerObjectiveMaxRank, 160)
  LibDD:UIDropDownMenu_SetText(dropDownRankerObjectiveMaxRank, "Your objective: rank " .. rankerObjectiveScenario)
  LibDD:UIDropDownMenu_Initialize(dropDownRankerObjectiveMaxRank, function (self, level, menuList)
    local info = LibDD:UIDropDownMenu_CreateInfo()
    local rankName
    for i = 1, maxObtainableRankScenario do
      rankName = GetPVPRankInfo(i + 4)
      info.func = self.SetValue
      info.text, info.arg1, info.checked = "rank " .. i .. " (" .. rankName .. ")", i, i == rankerObjectiveScenario
      info.icon = format("%s%02d", "Interface\\PvPRankBadges\\PvPRank", i)
      info.isNotRadio = false
      LibDD:UIDropDownMenu_AddButton(info)
    end
  end)
  function dropDownRankerObjectiveMaxRank:SetValue(newValue)
    rankerObjectiveScenario = newValue
    LibDD:UIDropDownMenu_SetText(dropDownRankerObjectiveMaxRank, "Your objective: rank " .. rankerObjectiveScenario)
    local rankName, rankNumber = GetPVPRankInfo(rankerObjectiveScenario + 4)
    local rankIcon = format("%s%02d", "Interface\\PvPRankBadges\\PvPRank", rankNumber)
    Ranker.Texture2:SetTexture(rankIcon)
    CloseDropDownMenus()
    Ranker:Predict()
  end
  Ranker.dropDownRankerObjectiveMaxRank = dropDownRankerObjectiveMaxRank
  ...
  Ranker.dropDownRankerLimit = dropDownRankerLimit
  -- Dropdown list for the player's honor limit at rank 14 for use during scenario modeling
  local dropDownRankerLimitMaxRank = LibDD:Create_UIDropDownMenu("RankerLimitMaxRank", mainFrame.ScrollChild)
  dropDownRankerLimitMaxRank:SetPoint("TOPRIGHT", 12, -21)
  LibDD:UIDropDownMenu_SetWidth(dropDownRankerLimitMaxRank, 180)
  LibDD:UIDropDownMenu_SetText(dropDownRankerLimitMaxRank, "Your honor limit: " .. FormatLargeNumber(rankerLimitScenario))
  LibDD:UIDropDownMenu_Initialize(dropDownRankerLimitMaxRank, function (self, level, menuList)
    local info = LibDD:UIDropDownMenu_CreateInfo()
    local limit = maxObtainableRankScenario
    for i = 1, limit do
      info.func = self.SetValue
      if i < limit then
        info.text, info.arg1, info.checked = FormatLargeNumber(honorIncrements[i]) .. " to " .. FormatLargeNumber(honorIncrements[i + 1] - 1) .. " honor", honorIncrements[i], honorIncrements[i] == rankerLimitScenario
      else
        info.text, info.arg1, info.checked = FormatLargeNumber(honorIncrements[i]) .. " or more honor", honorIncrements[i], honorIncrements[i] == rankerLimitScenario
      end
      LibDD:UIDropDownMenu_AddButton(info)
    end
  end)
  function dropDownRankerLimitMaxRank:SetValue(newValue)
    rankerLimitScenario = newValue
    LibDD:UIDropDownMenu_SetText(dropDownRankerLimitMaxRank, "Your honor limit: " .. FormatLargeNumber(rankerLimitScenario))
    CloseDropDownMenus()
    Ranker:Predict()
  end
  Ranker.dropDownRankerLimitMaxRank = dropDownRankerLimitMaxRank
  ...
  toggleButton:HookScript("OnMouseDown", function (self, button)
    if button == "LeftButton" then
      Ranker.ToggleWindow()
    elseif button == "RightButton" then
      Ranker.OpenConfigurationOptions()
  ...
  whatIfButton:HookScript("OnMouseDown", function (self, button)
    if button == "LeftButton" then
      Ranker.ToggleWhatIfScenarioModeling()
    elseif button == "RightButton" then
      Ranker.OpenConfigurationOptions()
    end
  end)
...
function Ranker:ToggleWhatIfScenarioModeling()
  ...
  if whatIfScenario == nil or whatIfScenario == false then
    whatIfScenario = true
    Ranker.dropDownRankerObjective:Hide()
    Ranker.dropDownRankerObjectiveMaxRank:Show()
    Ranker.dropDownRankerLimit:Hide()
    Ranker.dropDownRankerLimitMaxRank:Show()
    Ranker.WhatIfButton:SetText(addonWhatIfButtonTextHide)
...
function Ranker:ResetToCurrentSituationAndPredict()
  ...
  Ranker.dropDownRankerObjectiveMaxRank:Hide()
  Ranker.dropDownRankerObjective:Show()
  Ranker.dropDownRankerLimitMaxRank:Hide()
  Ranker.dropDownRankerLimit:Show()
...
function Ranker:WhatIfRankProgressBoxChanged(frame, key)
  ...
  if value ~= "0.00%" then
    value = gsub(value, "%%", "")
    if not tonumber(value) or tonumber(value) < 0 or tonumber(value) >= 100 then
      frame:SetText("0.00%")
      return
    end
  ...
  if key == "DOWN" then
    value = value - 0.01
    if value < 0 then value = 99.99 end
  end
  if key == "RIGHT" then
    value = value + 1
    if value > 99.99 then value = value - 100 end
  end
  if key == "LEFT" then
    value = value - 1
    if value < 0 then value = value + 100 end
  end
  if key == nil then
...
-- Set the tooltip for data brokers
function Ranker:UpdateDataBroker(tooltip)
  if worldLoaded and addonLoaded and interfaceLoaded then
    -- Data broker text
    local text = ""
    if dataBrokerNeedMoreHK == true then
      text = "" .. dataBrokerHK .. " more HK"
    end
    if dataBrokerNeedMoreHonor == true then
      if text ~= "" then text = text .. "; " end
      text = text .. "" .. FormatLargeNumber(dataBrokerHonor) .. " more honor"
    end
    if dataBrokerNeedMoreHK ~= true and dataBrokerNeedMoreHonor ~= true then
      text = "Click to open Ranker"
    end
    LibDBRanker.text = text
  end
end
-----
...
function Ranker:CreateConfigurationOptions()
  ...
  local category, _ = Settings.RegisterCanvasLayoutCategory(RankerFrame, RankerFrame.name)
  Settings.RegisterAddOnCategory(category)
  RankerFrame.categoryId = category:GetID()
  -- Title
  ...
  -- Option to hide the helpful information about the ranking system
  ...
  -- Option to stop auto promote in battlegrounds
  RankerFrame.buttonAutoPromote = CreateFrame("CheckButton", "default", RankerFrame, "UICheckButtonTemplate")
  RankerFrame.buttonAutoPromote:SetPoint("TOPLEFT", 20, -150)
  RankerFrame.buttonAutoPromote.Text:SetText("Automatically promote all players to raid assistant in battlegrounds")
  RankerFrame.buttonAutoPromote:SetChecked(rankerOptions.autoPromote)
  RankerFrame.buttonAutoPromote:HookScript("OnClick",
    function ()
      rankerOptions.autoPromote = not rankerOptions.autoPromote
      optionsChanged = true
  end)
end
-- Open the configuration panel
function Ranker:OpenConfigurationOptions()
  Settings.OpenToCategory(RankerFrame.categoryId)
end
-----
-- Utilities
        ...
        if message ~= nil then
          ...
          if name ~= playerName then
            ...
            if Ranker:VersionCheck(version, rankerVersion) == true then
              ...
              -- Warn the user 60 minutes after receiving a message from a player with a newer version (and reset the 60 minute timer on each loading screen or interface reload)
              ...
              elseif newVersionDetected == nil then
                if newVersionDetectionTimer == nil then
                  newVersionDetectionTimer = NewTicker(timerDuration * 6, Ranker.VersionDetectionResetTimer)
                end
              end
...
-- Reset the version check after x minutes to avoid spamming the player
...
function Ranker:Predict(overrideHK, overrideHonor, overrideRank, overrideProgress)
  ...
  if overrideHK and overrideHonor and overrideRank and overrideProgress then
    ...
    dataBrokerCurrentHonor = overrideHonor
  ...
  local l_rankerLimit = rankerLimit
  local l_rankerObjective = rankerObjective
  -- What if scenario modeling
  if whatIfScenario then
    l_maxObtainableRank = rankerObjectiveScenario
    l_rankerLimit = rankerLimitScenario
    l_rankerObjective = rankerObjectiveScenario
  ...
  if (overrideRank or playerRank) >= l_rankerObjective then
    ...
    Ranker:SetFrameText(Ranker.FrameContent, "Your objective was to be a " .. GetPVPRankInfo((l_rankerObjective or overrideRank or playerRank) + 4) .. ".")
  ...
  -- Start the prediction for players below rank l_maxObtainableRank
  ...
  if not optionsChanged and (scenarioHK and scenarioHK == overrideHK and scenarioHonor and scenarioHonor == overrideHonor and scenarioRank and scenarioRank == overrideRank and scenarioProgress and scenarioProgress == overrideProgress and scenarioObjective and scenarioObjective == l_rankerObjective and scenarioLimit and scenarioLimit == l_rankerLimit) then return end
  scenarioHK = overrideHK
  scenarioHonor = overrideHonor
  scenarioRank = overrideRank
  scenarioProgress = overrideProgress
  scenarioObjective = l_rankerObjective
  scenarioLimit = l_rankerLimit
  -- Verify honorable kill threshold
  local verb = "will"
  if overrideRank == 0 then overrideRank = 1 end
  if overrideHK < 15 then
    -- Data broker
    dataBrokerNeedMoreHK = true
    dataBrokerHK = 15 - overrideHK
    Ranker:UpdateDataBroker()
    verb = "can"
  ...
  for key = 1, #options do
    ...
    if options[key].honorRemains and options[key].honorRemains > 0 and options[key].rank <= l_rankerObjective and options[key].situation ~= "+1" and stop ~= true then
      ...
      if l_maxObtainableRank == options[key].rank then stop = true end
      if options[key].situation == "!!" then
        if lookupData.defaults.allowDecayPreventionHop == true and objectiveMet ~= true then
      ...
      if (options[key].situation == "!!" and lookupData.defaults.allowDecayPreventionHop == true) or options[key].situation ~= "!!" and objectiveMet ~= true then
    ...
    elseif (options[key].honorRemains and options[key].honorRemains <= 0) or (options[key].rank <= l_rankerObjective) and options[key].situation ~= "+1" and stop ~= true then
      ...
      if (options[options[key].number + 1] and options[options[key].number + 1].honorRemains > 0) or not options[options[key].number + 1] then
        if options[key].situation == "!!" then
          if lookupData.defaults.allowDecayPreventionHop == true then
            if options[key].rank >= l_rankerObjective then
              objectiveMet = true
              maxRank = options[key].rank
              verb = "will"
            end
  ...
  if #options > 0 then
    if objectiveMet == true then
      ...
      Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. "\n" .. "Your optimal path (least amount of time and honor) to rank " .. l_rankerObjective .. ":")
      ...
      if maxRank < l_rankerObjective then
        Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. "\n" .. "Obtaining rank " .. l_rankerObjective .. " is |cffe6cc80impossible with your honor limit|r!")
      else
        if weeksToGo == 0 then
          Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. "\n" .. "You " .. verb .. " be |cffe6cc80rank " .. maxRank .. "|r after the weekly reset.")
        ...
        elseif weeksToGo > 0 then
          Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. "\n" .. "Obtaining rank " .. l_rankerObjective .. " takes you |cffe6cc80" .. weeksToObjectiveText .. "|r after the weekly reset.")
  ...
  if not rankerOptions.hideHelpfulInformation then
    ...
    Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. " - Rank 7: |cffe6cc80" .. FormatLargeNumber(77500) .. "|r, or |cffe6cc80" .. FormatLargeNumber(142500) .. "|r, or |cffe6cc80" .. FormatLargeNumber(175000) .. "|r, or |cffe6cc80" .. FormatLargeNumber(256250) .. "|r honor")
    Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. " - Rank 8: |cffe6cc80" .. FormatLargeNumber(110000) .. "|r, or |cffe6cc80" .. FormatLargeNumber(175000) .. "|r, or |cffe6cc80" .. FormatLargeNumber(256250) .. "|r, or |cffe6cc80" .. FormatLargeNumber(337500) .. "|r honor")
    Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. " - Rank 9: |cffe6cc80" .. FormatLargeNumber(142500) .. "|r, or |cffe6cc80" .. FormatLargeNumber(256250) .. "|r, or |cffe6cc80" .. FormatLargeNumber(337500) .. "|r, or |cffe6cc80" .. FormatLargeNumber(418750) .. "|r honor")
    ...
    Ranker:SetFrameText(Ranker.FrameContent, Ranker:GetFrameText(Ranker.FrameContent) .. " - Rank 11: |cffe6cc80" .. FormatLargeNumber(256250) .. "|r, or |cffe6cc80" .. FormatLargeNumber(418750) .. "|r, or |cffe6cc80" .. FormatLargeNumber(500000) .. "|r honor")
...
function Ranker:ProjectSimplePathToObjectiveRank(rank, rankProgress, honorNeeded)
  ...
  if rank >= l_rankerObjective then return end
  ...
  while currentOption <= maxTries do
    ...
    for _, option in ipairs(options) do
      local rank
      if option.rankMin then rank = option.rankMin else rank = option.rank end
      if rank == l_rankerObjective then
      ...
      if rank > highestRank or (rank == highestRank and option.rankProgress > highestProgress) then
        bestOption = option
        highestRank = rank
...
function Ranker:ChooseYourOwnAdventure(rank, honor, progress)
  ...
  for num = 1, 5 do
    if rank + num - 1 <= min(l_maxObtainableRank + 4, 14) then
      ...
      if (predictedNewRank <= l_rankerObjective and (honorNeed <= l_rankerLimit)) or (honor >= honorNeed) and (honorNeed <= honor or honorNeed <= l_rankerLimit) then
        if num == 1 and rank + num <= l_rankerObjective then
        ...
        if (lookupData.defaults.allowDecayPreventionHop == true or (lookupData.defaults.allowDecayPreventionHop ~= true and situation ~= "!!")) and predictedNewRank <= l_maxObtainableRank then
          ...
          Ranker:OutputToChat("  ChooseYourOwnAdventure option: " .. num .. " (" .. situation .. ") need " .. honorNeed .. " honor (" .. honorRemains .. " more) for rank " .. (predictedNewRankMin or "nil") .. " and " .. (predictedNewProgressMin or "nil") .. ", or rank " .. predictedNewRank .. " and " .. predictedNewProgress .. ", or rank " .. (predictedNewRankMax or "nil") .. " and " .. (predictedNewProgressMax or "nil") .. ".", ddebug)
        end
      end
    end
  end
...
function Ranker:FindTheBestPossibleOption(entries)
  ...
  for i = 2, #entries do
    ...
    Ranker:OutputToChat("    FindTheBestPossibleOption: " .. currentEntry.weeksTotal .. " vs " .. bestEntry.weeksTotal .. ", " .. currentEntry.honorTotal .. " vs " .. bestEntry.honorTotal .. ", " .. currentEntry.honorRemains .. " vs " .. bestEntry.honorRemains .. ", " .. currentEntry.rankTo .. " vs " .. bestEntry.rankTo .. ", " .. currentEntry.option .. " vs " .. bestEntry.option .. ", " .. #entries .. ", " .. currentEntry.rankFrom .. " and " .. currentEntry.progressFrom .. ", " .. currentEntry.rankTo .. " and " .. currentEntry.progressTo, ddebug)
...
function Ranker:ProjectOptimalPathToObjectiveRank(overrideRank, overrideProgress, overrideHonor)
  ...
  -- Data broker
  if bestEntry.weekText == "current week" then
    dataBrokerNeedMoreHonor = true
    dataBrokerHonor = bestEntry.honorRemains
    Ranker:UpdateDataBroker()
  end
  ...
  if (bestEntry.rankFrom == bestEntry.rankTo and bestEntry.progressFrom == bestEntry.progressTo) or (bestEntry.rankTo == l_rankerObjective) or (tries > maxTries) then
    return
  end
