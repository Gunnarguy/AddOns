...
function MI2_TooltipMouseoverUnit()
	...
	if UnitIsUnit("mouseover","pet") then return end
	...
	if MI2_GameTooltipNumLinesOriginal > 0 then
		MI2_CreateTooltip( unitId, unitLevel, "mouseover", isMob, (UnitIsWildBattlePet and UnitIsWildBattlePet("mouseover")) or ( UnitIsBattlePetCompanion and UnitIsBattlePetCompanion("mouseover") ) )
		MI2_HideTimer = 5
	end
end
...
local function MI2_EntryToString( type, data, mobData, unit )
	...
	elseif type == 3 and mobData.location then
		...
		local z
		if loc.m then z= C_Map.GetMapInfo(loc.m).name end
...
local function MI2_UpdateTooltip( mobData, mobName, unit, isPet )
	...
	for idx = 1, numEntries do
		...
		local txtWidth = entry:GetStringWidth() 
		local valWidth = entry.val:GetStringWidth()
	...
	if numExtraLines > 0 then
		...
		for idx = 1, numExtraLines do
			extraWidth = max(extraWidth,_G["MI2TT_ExtraInfo"..idx]:GetStringWidth()+(extraItemDataWidth or 0 ))
		end
	else
...
-- MI2_CreateTooltip()
--
function MI2_CreateTooltip( id, level, unit, isMob, isPet, unique )
	...
	then
		...
		MI2_MouseoverIndex = unitGuid -- must be set after calling MI2_RecordLocation
	end
...
--
function MI2_GameTooltipCleared()
...
--
function MI2_GameTooltipSetUnit(...)
...
function MI2_IsUnitClose(unit, level)
...
--
function MI2_GameTooltipOnUpdate()
	...
	if unit and isMob
	then
		MI2_CacheMobInfo(UnitGUID(unit),name,level,mobData,MI2_IsUnitClose(unit),UnitClassification(unit))
	end
