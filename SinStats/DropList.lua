...
local function AddTipText(self)
	...
	self.Tip:SetFont(Ns.ConfigDefaultFont, Ns.ConfigDefaultFontSize -4)
...
function Mixins.functions.DropListRow.Init(self, parent, rowdata)--, id)
	...
	if parent.Data == ListData.Fonts then
		self.Text:SetFont(Ns.LSM:Fetch("font", v), 12)
	else
		self.Text:SetFont(Ns.ConfigDefaultFont, 12)
	end
	self.Text:SetText(t)
	self:Show()
...
--[[
	...
	self.ProfileList = Ns:GetWidget("DropList", self, 4) -- 4 = entry in Ns.SettingsGroupOrder table
